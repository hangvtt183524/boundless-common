<template>
  <div class="inline-edit-cell">
    <div class="inline-edit-cell__value" v-show="!editing">
      <span>{{ displayValue }}</span>

      <v-tooltip right color="#223d70" light>
        <template v-slot:activator="{ on }">
          <v-icon @click="onEditClick" v-on="on"
                  class="inline-edit-cell__edit-icon"
          >$vuetify.icons.edit</v-icon>
        </template>

        <span>{{ tooltip ? tooltip : 'Click to update' }}</span>
      </v-tooltip>
    </div>

    <div class="inline-edit-cell__input" v-show="editing">
      <base-text-input class="base-text-input--small"
                       v-model="inputValue"
                       ref="input"
                       @enterKeyUp="onUpdateClick"
      >
        <v-icon v-show="!loading" @click="onUpdateClick"
                class="inline-edit-cell__input__check-icon"
        >check</v-icon>

        <v-progress-circular class="inline-edit-cell__loading-indicator"
                             v-if="loading" :size="16" indeterminate
        />

        <v-icon @click="onCloseClick"
                class="inline-edit-cell__input__close-icon"
        >close</v-icon>
