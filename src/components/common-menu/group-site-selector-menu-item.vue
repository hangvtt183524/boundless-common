<template>
  <div class="node-selector-item"
       :class="{'node-selector-item--no-children': !hasChildren,
       'node-selector-item--inner': depth > 0,
       'node-selector-item--open': childrenVisible,
       'node-selector-item--active': node.id === selectedNodeId}"
       v-show="isVisible"
  >
    <div class="node-selector-item__header">
      <div class="node-selector-item__header__expand-trigger"
           v-if="hasChildren"
           @click="onExpandTriggerClick"
      >
        <v-icon :class="{'show-icon': !childrenVisible, 'hide-icon': childrenVisible}">
          {{ childrenVisible ? 'remove' : 'add' }}
        </v-icon>
      </div>

      <div class="node-selector-item__header__select-trigger"
           @click="onSelectTriggerClick"
      >
        <div class="node-selector-item__header__select-trigger__title">
          | {{ node.name }}
        </div>
      </div>
    </div>
    ...
    <transition name="slide">
      <div class="node-selector-item__children-container"
           v-if="hasChildren"
           v-show="childrenVisible"
      >
        <group-site-selector-menu-item
            class="node-selector-item__child-item"
