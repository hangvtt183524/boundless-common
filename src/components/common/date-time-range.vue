<template>
  <div class="daterange-picker" ref="datetimerange">
    <div class="daterange-picker-label">Time Range</div>

    <div class="daterange-picker-toolbar">
      <ul class="datepicker-quickpicks" v-if="!isCustomShow">
        <li class="datepicker-quickpick"
      ...
      <div class="datepicker-quickpick">
        <a @click="showHidePopup">
          <div class="picker-custom-value" v-if="isCustomShow">
            {{ `${backupDateTime[0]} (${backupDateTime[1]}) - ${backupDateTime[2]} (${backupDateTime[3]})` }}
          </div>

          <span :class="customLabel !== 'custom' ? 'picker-custom-value' : ''"
                v-if="!isCustomShow"
          >
            {{ customLabel }}
          </span>
    ...
    <div class="daterange-picker-customize" v-show="isPopupShow">
      <div class="customize-type-tabs">
        <div class="tab-item"
      ...
      <div class="daterange-picker-absolute-container" v-show="selectedTabItem === 0">
        ...
        <div class="daterange-datetime-container">
          <v-layout wrap row>
          ...
          <v-layout wrap row>
            <v-flex pt-2 pb-2>
              <label class="error-message">{{ validationErrorMessage }}</label>
            </v-flex>
          </v-layout>
        </div>

        <div class="daterange-action-button-container">
          <v-layout row>
            <v-flex class="text-xs-right pr-2 sm-2">
              <v-btn color='error' @click.stop='cancelDates'>
                Cancel
              </v-btn>

              <v-btn :disabled="!isValidateDateTime"
                     color='primary'
      ...
      <div class="daterange-picker-relative-container" v-if="selectedTabItem === 1">
        <div class="relative-row"
             v-for="relative in relativeValueList"
             :key="relative"
        >
          <div class="relative-label">{{ relative.label }}</div>
