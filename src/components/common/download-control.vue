    ...
    >
      ...
      <v-card class="download-menu">
        ...
        <v-layout row wrap>
          ...
          <v-flex xs12 sm6 md4 lg3 xl2 class="mb-1"
                  v-for="column in exportColumns"
                  :key="column.value"
          >
            <v-checkbox
                v-if="!isTree && column.text && column.text !== 'actions' && column.text !== 'Reload'"
                :label="column.text"
        ...
        <v-layout row wrap>
          <v-flex xs12 class="mb-1">
            <v-checkbox label="Send link by mail" v-model="isSendEmail" />
          </v-flex>
  ...
  methods: {
    disableDownloadBtn () {
      for (let i = 0; i < this.exportColumns.length; i++) {
        if (this.exportColumns[i].text && this.exportColumns[i].selected && this.exportColumns[i].text !== 'actions' && this.exportColumns[i].text !== 'Reload') return false
      }
      return true
    },
    showModifyMenu () {
